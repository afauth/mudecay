...
for event in range(necessarySamples):
...
'''
waveformsList = np.delete( arr=waveformsList, obj=0, axis=0 )
...
df = pd.DataFrame(
        data=waveformsList.T, 
        columns=[ ('event_'+str(i)) for i in range(necessarySamples) ]  
                 )
